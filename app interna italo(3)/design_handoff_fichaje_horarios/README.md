...
# 🟡 PARTE 3 — Horas extras (nueva, agregada después de la Parte 1)

> Vive dentro de la misma tab **Horarios** del admin. Se implementa después de Parte 1; no depende de Parte 2 (QR).

## P3.1 — Cambio de navegación: "Horarios" ahora tiene una pantalla de entrada

**Archivo de referencia:** `admin-movil.jsx` (componente `HorariosHome` + `Shell`)

La tab **Horarios** deja de abrir directo el ABM de turnos. Ahora abre una pantalla intermedia con **2 opciones** (mismo patrón que "Encuestas": una pantalla lista con acciones, no tabs internas):

1. **Manejo de turnos** → abre exactamente la pantalla de la Parte 1 (P1.1), con un botón **‹ Horarios** arriba a la izquierda para volver.
2. **Manejo de horas extras** → abre la nueva pantalla P3.2, con el mismo botón de volver.

Cada tarjeta: ícono en burbuja + título + bajada de una línea + chevron. Ver `admin-movil.jsx` → `HorariosHome`.

## P3.2 — Pantalla: Horas extras (Admin, mobile)

**Archivo de referencia:** `horas-extras.jsx`

### Propósito
El admin ve, para cada empleado, el total de horas extra trabajadas en un **rango de fechas**, y puede **liquidarlas** (marcar como pagadas — la app solo **registra** la acción, no procesa el pago real) o ver el **detalle día por día** de esas horas.

### Layout (de arriba hacia abajo)
1. **Header**: botón volver (‹ Horarios) + título "Horas extra" + subtítulo.
2. **Selector de rango de fechas — SIEMPRE visible y con valor, nunca vacío/opcional**:
   - Dos chips de preset: **"Semana pasada"** (última semana cerrada — default al entrar) / **"Semana actual"**.
   - Dos inputs de fecha ("Desde" / "Hasta") para un rango custom; ambos `required`, no hay forma de dejarlos en blanco.
   - Cambiar el rango recalcula todos los totales de la lista y del detalle.
3. **Buscador de empleado** (input con lupa, mismo patrón que P1.4 — en implementación real, reusar el selector de usuarios ya existente).
4. **Filtro por rol** (dropdown, mismo patrón visual que el filtro de "Sede" en P1.1): `Todos` + roles (`Vendedor`, `Encargado`, `Depósito`, `Administrativo` — roles reales salen del sistema).
5. **Lista de empleados** (uno por fila): avatar (iniciales), nombre, rol; a la derecha el total de horas extra en el rango (`Xh`, color ámbar) y la etiqueta "extra"; botón **Liquidar** (pill, ícono de billete) al final de la fila. Tocar la fila (fuera del botón) abre el detalle; tocar **Liquidar** ejecuta la liquidación directamente desde la fila.
6. **Barra inferior (info)**: "`N` empleados · `Xh` horas extra en el rango".

### P3.3 — Detalle de empleado (bottom sheet)
Mismo patrón visual que la hoja de edición de turno (P1.3): sube desde abajo, backdrop oscuro.
- Nombre + rol.
- **Total destacado** del rango elegido, en una tarjeta con fondo celeste suave (grande, centrado).
- **"Distribución por día"**: lista día por día dentro del rango seleccionado (día de semana + `dd/mm`, horas a la derecha; `—` si ese día no tuvo horas extra).
- Botón **Liquidar horas extra** (primario, full width, ícono de billete) al pie.

### P3.4 — Liquidar (registrar pago)
- Acción disponible en la fila de la lista **y** dentro del detalle (mismo resultado).
- Al confirmar: se muestra un **toast** ("Horas extra de {nombre} liquidadas") y se cierra el detalle si estaba abierto.
- **No hay estado persistente de pendiente/liquidado** en este alcance — la acción solo se registra (el toast es la confirmación). Si a futuro se pide trazabilidad (quién liquidó, cuándo, qué rango), agregar un registro de liquidaciones en el backend; no está en este diseño.
- La app **no procesa el pago**: liquidar es un registro interno de que ese pago ya se hizo por fuera del sistema.

### P3.5 — Modelo de datos (Parte 3)
```
Empleado { nombre, rol }              // rol: string libre (ej. Vendedor, Encargado, Depósito, Administrativo)
HorasExtraDia { nombre, fecha, horas } // horas extra ya calculadas por día, por empleado (viene resuelto del backend)
Liquidacion { nombre, rango: {desde, hasta}, fecha_accion }  // si se decide persistir el registro de la acción
```
En el prototipo, el cálculo de "qué cuenta como hora extra" **ya viene resuelto** (dato de ejemplo por día); la regla real (ej. >40hs semanales, o exceso sobre el turno pactado) la define el backend — el front solo suma los valores diarios dentro del rango elegido.

### P3.6 — Filtros: comportamiento
- **Rango de fechas**: obligatorio, siempre aplicado, con preset "semana pasada"/"semana actual" o custom.
- **Rol**: `Todos` (default) | un rol. 
- **Búsqueda**: por nombre, case-insensitive. Se combina (AND) con rol y rango.
...
🟡 **Parte 3**
- Entrar a la tab Horarios → pantalla con 2 tarjetas → elegir "Manejo de turnos" o "Manejo de horas extras" → volver con el botón ‹ Horarios.
- Cambiar preset o fechas custom del rango → recalcula lista y (si está abierto) el detalle.
- Buscar / filtrar por rol → filtra la lista (AND con el rango).
- Tocar fila → abre detalle con distribución día por día.
- Tocar "Liquidar" (fila o detalle) → toast de confirmación, cierra el detalle.
...
🟡 **Parte 3 — Horas extras**
- `screen` en `Shell` (`home`/`turnos`/`horas`) controla qué se ve dentro de la tab Horarios.
- En `HorasExtrasScreen`: `q` (búsqueda), `role` (filtro), `range` (`{from, to}` — nunca null), `detail` (empleado abierto | null), `toast`.
- Derivados: `days` (fechas dentro del rango), `rows` (empleados filtrados + total calculado), `totalAll`.
...
| `horas-extras.jsx` | Pantalla de Horas extras (lista, filtros, detalle, liquidar) | 🟡 3 |
